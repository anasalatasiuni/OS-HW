...
{
  prime_counter_request* arguments = ((prime_counter_request*) arg);
  int *ptr = malloc(sizeof(int));
  *ptr = primes_count_in_interval(arguments->start,arguments->finish);
  return ((void *) ptr);
}
...
int main(int argc, char *argv[])
{
  ...
  for (int i = 0; i < n_threads; i++)
  {
    ...
    requests[i].finish = ((i+1)*segment_size < n-1? (i+1)*segment_size: n-1);
    pthread_create( &threads[i], NULL, prime_counter, (void *) (&requests[i]));
  }

  for (int i = 0; i < n_threads; i++)
  {
    int resp_code = pthread_join(threads[i],&results[i]);
    if(resp_code != 0 )
